...
const terraform_1 = __importDefault(__nccwpck_require__(3620));
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const rawVariables = core.getMultilineInput('tfVars');
...
class Terraform {
     ...
     */
    _checkWorkspace(workspace) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (workspace.includes(' ')) {
                    throw new Error(`Workspace name should not contain spaces.`);
                }
                const res = yield this.axios.get(`/organizations/${this.org}/workspaces/${workspace}`);
     ...
     */
    _getConfigVersionStatus(configVersionId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const res = yield this.axios.get(`/configuration-versions/${configVersionId}`);
     ...
     */
    _createConfigVersion(workspaceId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const body = {
                    data: {
                ...
                const res = yield this.axios.post(`/workspaces/${workspaceId}/configuration-versions`, JSON.stringify(body));
     ...
     */
    _uploadConfiguration(configId, uploadUrl, filePath) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield this.axios.put(uploadUrl, fs_1.default.createReadStream(filePath), {
                    headers: { 'Content-Type': `application/octet-stream` }
                });
                // Wait for TFC to update to avoid ECONNRESET - this could be neater.
                core.info(`Sleeping for TFC ${this.tfcInterval} ms.`);
                yield this._sleep(this.tfcInterval);
                let status = yield this._getConfigVersionStatus(configId);
                let counter = 0;
                while (status === 'pending') {
                    if (counter < this.retryLimit) {
                        yield this._sleep(this.uploadRetryInterval);
                        status = yield this._getConfigVersionStatus(configId);
                        counter += 1;
                    }
                    else {
     ...
     */
    _run(workspaceId, identifier, variables) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const run = {
                    data: {
                ...
                const res = yield this.axios.post('/runs', JSON.stringify(run));
                if (!res.data || !res.data.data) {
                    throw new Error('No data returned from request.');
                }
                else if (!res.data.data.id) {
                ...
                if (err === null || err === void 0 ? void 0 : err.response) {
                    ...
                    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                    // @ts-ignore
                    (err === null || err === void 0 ? void 0 : err.response.data) ? err === null || err === void 0 ? void 0 : err.response.data.errors : null)}`;
                }
    ...
    _getStatus(runId) {
        return __awaiter(this, void 0, void 0, function* () {
            const res = yield this.axios.get(`/runs/${runId}`);
            return res.data.data.attributes.status;
        });
    }
    /**
     ...
     */
    run(workspace, filePath, identifier, variables) {
        return __awaiter(this, void 0, void 0, function* () {
            const workspaceId = yield this._checkWorkspace(workspace);
            const { id, uploadUrl } = yield this._createConfigVersion(workspaceId);
            yield this._uploadConfiguration(id, uploadUrl, filePath);
            // eslint-disable-next-line prefer-const
            let { runId, status } = yield this._run(workspaceId, identifier, variables);
            if (this.awaitApply) {
                status = yield this._poll(runId);
            }
            return { runId, status };
        });
    }
